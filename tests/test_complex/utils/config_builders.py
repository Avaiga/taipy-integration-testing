...
def build_churn_classification_required_file_paths():
    csv_path_inp = os.path.join(
        pathlib.Path(__file__).parent.resolve(), "../../shared_test_cases/data_sample/churn.csv"
    )
    return csv_path_inp
...
def build_churn_classification_config():
    ...
    forecast_dataset = Config.configure_data_node(
        id="forecast_dataset", scope=Scope.PIPELINE, cacheable=True, validity_period=dt.timedelta(days=1)
    )

    roc_data = Config.configure_data_node(
        id="roc_data", scope=Scope.PIPELINE, cacheable=True, validity_period=dt.timedelta(days=1)
    )

    score_auc = Config.configure_data_node(
        id="score_auc", scope=Scope.PIPELINE, cacheable=True, validity_period=dt.timedelta(days=1)
    )

    metrics = Config.configure_data_node(
        id="metrics", scope=Scope.PIPELINE, cacheable=True, validity_period=dt.timedelta(days=1)
    )
    ...
    results = Config.configure_data_node(
        id="results", scope=Scope.PIPELINE, cacheable=True, validity_period=dt.timedelta(days=1)
    )
