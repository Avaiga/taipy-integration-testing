...
from taipy.config import Config, Scope, Frequency

from .algos import *


def build_complex_config():
    csv_path_inp, excel_path_inp, csv_path_sum, excel_path_sum, excel_path_out, csv_path_out = build_complex_required_file_paths()
...
def build_complex_required_file_paths():
    csv_path_inp = os.path.join(pathlib.Path(__file__).parent.resolve(),
                                "../../shared_test_cases/data_sample/example.csv")
    excel_path_inp = os.path.join(pathlib.Path(__file__).parent.resolve(),
                                  "../../shared_test_cases/data_sample/example.xlsx")

    csv_path_sum = os.path.join(pathlib.Path(__file__).parent.resolve(),
                                "../../shared_test_cases/data_sample/sum.csv")
    excel_path_sum = os.path.join(pathlib.Path(__file__).parent.resolve(),
                                  "../../shared_test_cases/data_sample/sum.xlsx")

    excel_path_out = os.path.join(pathlib.Path(__file__).parent.resolve(),
                                  "../../shared_test_cases/data_sample/res.xlsx")
    csv_path_out = os.path.join(pathlib.Path(__file__).parent.resolve(),
                                "../../shared_test_cases/data_sample/res.csv")
...
def build_churn_classification_config():
    ...
    pipeline_preprocessing = Config.configure_pipeline(id="pipeline_preprocessing",
                                                       task_configs=[task_preprocess_dataset,
                                                                     task_create_train_test])

    pipeline_train_baseline = Config.configure_pipeline(id="pipeline_train_baseline",
                                                        task_configs=[task_train_model_baseline])
